            ...
            allusers.map((chat, index) => {
              return (
                <NavLink to={'/' + chat?.userDetails?._id} key={chat?._id} className={({ isActive }) =>
                  `flex items-center gap-2 py-5 px-2 cursor-pointer rounded my-1 mx-1 ${isActive
                    ? darkMode
                  ...
                  <div >
                    ...
                    <div className='text-slate-500 text-xs flex items-center gap-1'>
                      ...
                      <p className={`text-ellipsis line-clamp-1 ${darkMode ? "text-slate-100" : "text-gray-600"}`}>
                        {Typing.includes(chat?.userDetails?._id) ? (
                          <span className="text-blue-500">Typing...</span>
                        ) : (
                          <span>{chat?.lastmsg?.text}</span>
                        )}
                      </p>
                    </div>
